...
public class HttpSteps {
    ...
    private static final String RESPONSES_DIR = "data/responses/";
    ...
    @Then("check response {int} with body from file {word}")
    public void checkResponseWithBodyFromFile(int httpStatus, String fileName) {
        String expectedResponse = DataReader.readResource(
                testContext.getParam("parentDir") + fileName,
                testContext.getParams());
        checkResponse(httpStatus, expectedResponse);
    }

    @Then("check response is {word} with status {int}")
    public void checkResponseIsWithStatus(String fileName, int httpStatus, List<Map<String, String>> parametersList) {
    ...
    @Then("set header {word} to {string}")
    public void setHeaderTo(String headerName, String headerValue) {
        testContext.addMobileRequestHeader(headerName, DataReader.substituteParamsInString(headerValue,
                testContext.getParams()));
    }

    @And("save data from response")
    public void saveDataFromResponse(Map<String, String> parameters) {
        parameters.keySet().stream().filter(key -> !key.equals("contextName")).forEach(key ->
                testContext.addParam(key, JsonPath.read(testContext.getResponseBody(), parameters.get(key))));
    }
    ...
    public void sendDeletePet(String petId) {
        ...
        Assert.assertTrue(String.format("Error %d was occured during deteting of pet with id %s.",
                ...
                testContext.getResponse().isSuccess());
        logger.info("Pet with id %d was successfully deleted", substitutedPetId);
    }
