...
public class PlaceholderProcessor {
    ...
    private static final Pattern patternUuid = Pattern.compile("(<(randomUuid)>)");
    private static final Pattern patternId = Pattern.compile("(<(uniqueId)>)");
    private static final Pattern patternAlphanumeric = Pattern.compile("(<(randomAlphaNum), (?<length>\\d*)>)");
    private static final Pattern patternAlpha = Pattern.compile("(<(randomAlpha), (?<length>\\d*)>)");
    ...
    public static String replaceHoldersInValues(String value) {
        ...
        value = processUuidPlaceholder(value);
        value = processUniqueIdPlaceholder(value);
        value = processAlNumPlaceholder(value);
        value = processAlphaPlaceholder(value);
        return value;
    }
    ...
    /**
     * Replace value with placeholder "<randomUuid>" with generated Uuid
     */

    private static String processUuidPlaceholder(String value) {
        Matcher matcher = patternUuid.matcher(value);
        while (matcher.find()) {
            String holder = matcher.group(0);
            String holderVal;
            holderVal = UUID.randomUUID().toString();
            value = value.replace(holder, holderVal);
        }
        return value;
    }

    /**
     * Replace value with placeholder "<uniqueId>" with generated Uuid
     */

    private static String processUniqueIdPlaceholder(String value) {
        Matcher matcher = patternId.matcher(value);
        while (matcher.find()) {
            String holder = matcher.group(0);
            String holderVal;
            holderVal = Long.toString(System.currentTimeMillis());
            value = value.replace(holder, holderVal);
        }
        return value;
    }
    ...
    /**
     * Replace value with placeholder "<randomAlphaNum, ...>" with generated alphanumeric
     */

    private static String processAlNumPlaceholder(String value) {
        Matcher matcher = patternAlphanumeric.matcher(value);
        while (matcher.find()) {
            String holder = matcher.group(0);
            String length = matcher.group("length");
            String holderVal;
            holderVal = RandomStringUtils.randomAlphanumeric(Integer.parseInt(length));
            value = value.replace(holder, holderVal);
        }
        return value;
    }
    ...
    /**
     * Replace value with placeholder "<randomAlpha, ...>" with generated alphabetic
     */

    private static String processAlphaPlaceholder(String value) {
        Matcher matcher = patternAlpha.matcher(value);
        while (matcher.find()) {
            String holder = matcher.group(0);
            String length = matcher.group("length");
            String holderVal;
            holderVal = RandomStringUtils.randomAlphabetic(Integer.parseInt(length));
            value = value.replace(holder, holderVal);
        }
        return value;
    }
